...
ENVS_DIR=$(conda config --show envs_dirs | awk '/^  -/{print $2; exit}')
...
source ${CONDA_BASE}/bin/activate ${ENVS_DIR}/BindCraft || { echo -e "Error: Failed to activate the BindCraft environment."; exit 1; }
[ "$CONDA_DEFAULT_ENV" = "BindCraft" ] || { echo -e "Error: The BindCraft environment is not active."; exit 1; }
echo -e "BindCraft environment activated at ${ENVS_DIR}/BindCraft"
...
pip3 install git+https://github.com/LeonardoTredese/ColabDesign.git --no-deps || { echo -e "Error: Failed to install ColabDesign"; exit 1; }
